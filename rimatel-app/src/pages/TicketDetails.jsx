    ...
    return (
      <div className="max-w-4xl mx-auto p-6">
        <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-lg">
          ...
          <p>{error || (t('ticket.notFound') || 'Ticket non trouvé')}</p>
        </div>
        <button
    ...
    <div className="max-w-4xl mx-auto p-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-500 mt-1">
              {t('ticket.createdAt')}: {new Date(ticket.created_at).toLocaleDateString(locale)}
            </p>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          <h2 className="text-xl font-semibold text-gray-900 mb-4 flex items-center gap-2">
            ...
            {t('ticket.clientInfo') || 'Informations Client'}
          </h2>
          
          <div className="space-y-3">
            <div>
              <label className="text-sm text-gray-500">{t('ticket.subscriberNumber') || "N° d'abonné"}</label>
            ...
            {ticket.client_name && (
              <div>
                <label className="text-sm text-gray-500">{t('ticket.clientName') || 'Nom du client'}</label>
            ...
            <div>
              <label className="text-sm text-gray-500 flex items-center gap-1">
                ...
                {t('ticket.location') || 'Localisation'}
              </label>
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          <h2 className="text-xl font-semibold text-gray-900 mb-4 flex items-center gap-2">
            ...
            {t('ticket.technicalInfo') || 'Informations Techniques'}
          </h2>
          
          <div className="space-y-3">
            <div>
              <label className="text-sm text-gray-500">{t('ticket.subscriptionType') || "Type d'abonnement"}</label>
              <p className="font-medium text-gray-900">{ticket.subscription_type}</p>
            </div>
            
            {ticket.complaint_type && (
              <div>
                <label className="text-sm text-gray-500">{t('ticket.complaintType') || 'Type de réclamation'}</label>
            ...
            <div>
              <label className="text-sm text-gray-500 flex items-center gap-1">
                ...
                {t('ticket.createdAt') || 'Date de création'}
              </label>
              <p className="font-medium text-gray-900">
            ...
            {ticket.users && (
              <div>
                <label className="text-sm text-gray-500">{t('ticket.createdBy') || 'Créé par'}</label>
      ...
      <div className="bg-white rounded-lg shadow-md p-6 mt-6">
        <h2 className="text-xl font-semibold text-gray-900 mb-4 flex items-center gap-2">
          ...
          {t('ticket.problemDescription') || 'Description du problème'}
        </h2>
      ...
      <div className="mt-6 bg-blue-50 border border-blue-200 rounded-lg p-6">
        <h2 className="text-xl font-semibold text-blue-900 mb-4 flex items-center gap-2">
          {t('ticket.comments') || commentsLabel}
        ...
        {commentsLoading ? (
          ...
          <p className="text-blue-800">{t('ticket.noComments') || (i18n?.language?.startsWith('en') ? 'No comments yet.' : i18n?.language?.startsWith('ar') ? 'لا توجد تعليقات بعد.' : 'Aucun commentaire.')}</p>
        ) : (
              ...
              <li key={h.id} className="border border-blue-200 rounded-lg p-4 bg-white">
                <div className="flex justify-between items-center">
                  <div className="font-semibold text-gray-800">
                    {h.changed_by_name || (t('common.user') || (i18n?.language?.startsWith('en') ? 'User' : i18n?.language?.startsWith('ar') ? 'مستخدم' : 'Utilisateur'))}
                  </div>
      ...
      <div id="history" className="bg-white rounded-lg shadow-md p-6 mt-6">
        ...
        {historyLoading ? (
          ...
          <div className="space-y-8">
            ...
            <div>
              {history.length === 0 ? (
                <p className="text-gray-600">{t('ticket.noStatusChanges') || (i18n?.language?.startsWith('en') ? 'No status changes.' : i18n?.language?.startsWith('ar') ? 'لا تغييرات في الحالة.' : 'Aucune modification de statut.')}</p>
              ) : (
                <ul className="space-y-4">
                  {history.map((h) => (
                    <li key={h.id} className="border border-gray-200 rounded-lg p-4">
                      ...
                      {(h.changed_by_name) && (
                        <div className="mt-2 text-sm text-gray-600">
                          {(t('common.by') || (i18n?.language?.startsWith('en') ? 'By' : i18n?.language?.startsWith('ar') ? 'بواسطة' : 'Par'))}: {h.changed_by_name}
                        </div>
                      )}
                    </li>
                  ))}
