-- =====================================================
-- Mark 'assigné' tickets as 'en_retard' after 24h
-- Scheduled via pg_cron (hourly)
-- =====================================================

create or replace function mark_late_assigne_tickets()
returns void as $$
begin
  ...
  select u.id, 'status_change', 'assigné', 'en_retard', now(), null, 'System'
  from updated u;
end;
$$ language plpgsql;
...
select cron.schedule(
  'mark-late-assigne-hourly',
  '0 * * * *',
  'select mark_late_assigne_tickets()'
);
